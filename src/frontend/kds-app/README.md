# SmartMenu — Kitchen Display System
...
- **Auth**: `lib/auth-client.ts` → `createAuthApi('kds-app')` provee axios + interceptor JWT con refresh transparente
- **Errores**: `app/error.tsx` + `app/global-error.tsx` capturan throws + log a `lib/sentry.ts`
- **Login**: `<LoginScreen appKey="kds-app" .../>` componente compartido en `components/LoginScreen.tsx`
- **Roles aceptados**: Chef, KitchenStaff, Bartender (+ Admin)
