...
- **Auth**: `lib/auth-client.ts` → `createAuthApi('reservation-app')` provee axios + interceptor JWT con refresh transparente
- **Errores**: `app/error.tsx` + `app/global-error.tsx` capturan throws + log a `lib/sentry.ts`
- **Login**: `<LoginScreen appKey="reservation-app" .../>` componente compartido en `components/LoginScreen.tsx`
- **Roles aceptados**: Host, Hostess, Manager (+ Admin)
