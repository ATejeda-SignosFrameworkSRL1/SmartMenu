  ...
  async function submit() {
    ...
    try {
      ...
      if (err.code === 'EXPIRED') {
        ...
        toast.error(err.message || (mode === 'area' ? t('toastSendFailed') : t('toastConfirmFailed')));
      }
  ...
  if (step === 4 && result) {
    if (mode === 'area') {
      ...
      return (
        <div className="mx-auto max-w-lg rounded-3xl border border-primary/20 bg-warm-900/50 p-10 text-center shadow-2xl sm:p-12">
          ...
          <h2 className="mt-6 font-display text-3xl font-bold text-white">{t('areaSentTitle')}</h2>
          <p className="mt-3 text-warm-400">
            {t('areaSentBody', {
              zone: zoneName ? ` ${zoneName}` : '',
              date: new Date(date + 'T00:00:00').toLocaleDateString(dl, { weekday: 'long', day: 'numeric', month: 'long' }),
              time: to12h(areaTime),
            })}
          </p>
          <div className="mt-6 inline-block rounded-xl border border-primary/30 bg-warm-900 px-6 py-4">
            <p className="text-xs text-warm-500 uppercase tracking-wide">{t('trackingCode')}</p>
            <p className="text-2xl font-mono font-bold text-primary-light">{result.confirmationCode}</p>
          </div>
          <div className="mt-8 flex flex-col items-center gap-3">
            <Link href={`/seguimiento/${result.confirmationCode}`} className="btn-primary-lg w-full justify-center">{t('viewStatus')}</Link>
            <button onClick={reset} className="text-sm font-medium text-warm-400 transition-colors hover:text-primary-light">{t('anotherBooking')}</button>
          </div>
        </div>
      );
    }
    return (
      <div className="mx-auto max-w-lg rounded-3xl border border-primary/20 bg-warm-900/50 p-12 text-center shadow-2xl">
        <CheckCircle2 className="mx-auto h-20 w-20 text-primary-light" />
        <h2 className="mt-6 font-display text-3xl font-bold text-white">{result.status === 'Confirmed' ? t('confirmedTitle') : t('receivedTitle')}</h2>
        <p className="mt-3 text-warm-400">
          {result.status === 'Confirmed'
            ? t('weWaitYou', { date: new Date(date + 'T00:00:00').toLocaleDateString(dl, { weekday: 'long', day: 'numeric', month: 'long' }), time: to12h(selectedTime) })
            : t('youRequested', { date: new Date(date + 'T00:00:00').toLocaleDateString(dl, { weekday: 'long', day: 'numeric', month: 'long' }), time: to12h(selectedTime) })}
          {result.status !== 'Confirmed' && t('needsApproval')}
        </p>
        <div className="mt-6 inline-block rounded-xl border border-primary/30 bg-warm-900 px-6 py-4">
          <p className="text-xs text-warm-500 uppercase tracking-wide">{t('confirmationCode')}</p>
          <p className="text-2xl font-mono font-bold text-primary-light">{result.confirmationCode}</p>
        </div>
        <p className="mt-4 text-sm text-warm-400">
          {t('persons', { count: guests })} · {t('statusLabel')} <span className="text-white">{result.status === 'Confirmed' ? t('statusConfirmed') : t('statusPending')}</span>
        </p>
        <button onClick={reset} className="btn-primary mt-8">{t('anotherBooking')}</button>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-3xl rounded-3xl border border-warm-800 bg-warm-900/50 p-6 shadow-2xl backdrop-blur-sm sm:p-10">
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <div className="flex-1">
          <p className="text-xs font-bold uppercase tracking-wider text-primary-light">
            {t('stepOf', { current: mode === 'area' ? (step === 3 ? 2 : 1) : step, total: mode === 'area' ? 2 : 3 })}
          </p>
      ...
      {step === 1 && (
        <div className="space-y-5">
            ...
            <div className="rounded-xl border border-warm-700 bg-warm-800/50 p-3 sm:p-4">
              ...
              <div className="mb-3 flex items-center justify-between">
                ...
                <p className="text-base font-bold capitalize text-warm-100">
                  {calMonth.toLocaleDateString(dl, { month: 'long', year: 'numeric' })}
                </p>
                <button
                  type="button"
                  onClick={() => setCalMonth((m) => new Date(m.getFullYear(), m.getMonth() + 1, 1))}
          ...
          <div className="block">
            <span className="mb-1.5 flex items-center gap-1.5 text-sm font-medium text-warm-300">
              <Users className="h-4 w-4 text-primary-light" /> {t('guestsLabel')} *
            </span>
            <div className="flex items-center justify-between rounded-xl border border-warm-700 bg-warm-800/50 p-2">
              <button onClick={() => setGuests((g) => Math.max(minGuests, g - 1))} disabled={guests <= minGuests}
                className="h-12 w-12 rounded-lg bg-warm-800 text-2xl font-bold text-white hover:bg-warm-700 disabled:opacity-40 disabled:cursor-not-allowed" aria-label={t('lessGuests')}>−</button>
              ...
              <button onClick={() => setGuests((g) => Math.min(20, g + 1))} disabled={guests >= 20}
                className="h-12 w-12 rounded-lg bg-warm-800 text-2xl font-bold text-white hover:bg-warm-700 disabled:opacity-40 disabled:cursor-not-allowed" aria-label={t('moreGuests')}>+</button>
            </div>
            {mode === 'area' && (
              <p className="mt-1.5 text-xs text-primary-light">{t('areaMinNote', { min: AREA_MIN_GUESTS })}</p>
            )}
          </div>
          ...
          {zones.length > 0 && (
            <div className="block">
              <span className="mb-1.5 flex items-center gap-1.5 text-sm font-medium text-warm-300">
                ...
                {mode === 'area' ? t('zoneToReserve') : t('zonePreferred')}
                {mode === 'area'
                  ? <span className="text-rose-400">*</span>
                  : <span className="text-xs text-warm-500 font-normal">{t('optional')}</span>}
              </span>
              <div role="radiogroup" aria-label={mode === 'area' ? t('zoneToReserve') : t('zonePreferred')} className="flex flex-wrap gap-2">
                {mode !== 'area' && (
                  <ZoneChip selected={zoneId === null} onClick={() => setZoneId(null)} label={t('noPreference')} />
                )}
                {zones.map((z) => (
          ...
          {mode === 'area' && (
            <div className="block">
              <span className="mb-1.5 flex items-center gap-1.5 text-sm font-medium text-warm-300">
                <Clock className="h-4 w-4 text-primary-light" /> {t('hourLabel')} <span className="text-rose-400">*</span>
              </span>
              <div role="radiogroup" className="grid grid-cols-4 gap-2">
                {AREA_TIMES.map((tt) => {
                  const sel = areaTime === tt;
                  return (
                    <button
                      key={tt}
                      type="button"
          ...
          >
            {mode === 'area'
              ...
              : <>{t('seeSlots')} <ArrowRight className="h-5 w-5" /></>}
          </button>
        </div>
      )}
        ...
        <div className="space-y-5">
          ...
          {slotsError && (
            <div className="text-center py-8">
              ...
              <button onClick={fetchSlots} className="btn-primary">{t('retry')}</button>
            </div>
          )}
          ...
          {holding && (
            <p className="text-center text-sm text-warm-400 flex items-center justify-center gap-2">
              <Loader2 className="w-4 h-4 animate-spin" /> {t('reserving')}
            </p>
          )}
        </div>
      )}
        ...
        <div className="space-y-5">
          {mode === 'area' && (
            <div className="flex items-center gap-2 rounded-xl bg-primary/10 border border-primary/30 px-4 py-3 text-sm text-warm-200">
              ...
              <span>{t('areaSummary', {
                ...
                date: new Date(date + 'T00:00:00').toLocaleDateString(dl, { day: 'numeric', month: 'short' }),
                time: to12h(areaTime),
                count: guests,
              })}</span>
            </div>
          ...
          {selectedTime && (
            <div className="flex items-center gap-2 rounded-xl bg-primary/10 border border-primary/30 px-4 py-3 text-sm text-warm-200">
              ...
              <span>{t('tableHeldFor', { time: to12h(selectedTime) })} · {t('personsShort', { count: guests })}</span>
              {secondsLeft != null && <span className="ml-auto font-mono text-primary-light">{mmss}</span>}
            </div>
          )}
          <div className="grid gap-5 sm:grid-cols-2">
            <WarmField icon={<UserIcon className="w-4 h-4" />} label={t('nameLabel')} required>
          ...
          <WarmField icon={<PartyPopper className="w-4 h-4" />} label={t('occasionLabel')}>
          ...
          <WarmField label={t('specialRequestsLabel')}>
            <textarea rows={3} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder={t('specialRequestsPh')} className="warm-inp resize-none" />
          </WarmField>
          <button onClick={submit} disabled={submitting} className="btn-primary-lg mt-2 w-full disabled:cursor-not-allowed disabled:opacity-60">
            {submitting
              ? <><Loader2 className="h-5 w-5 animate-spin" /> {mode === 'area' ? t('sendingArea') : t('confirming')}</>
              : <>{mode === 'area' ? t('submitArea') : t('submitTable')} <ArrowRight className="h-5 w-5" /></>}
          </button>
        </div>
      )}
