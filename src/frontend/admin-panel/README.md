...
- **Auth**: `lib/auth-client.ts` → `createAuthApi('admin-panel')` provee axios + interceptor JWT con refresh transparente
- **Errores**: `app/error.tsx` + `app/global-error.tsx` capturan throws + log a `lib/sentry.ts`
- **Login**: `<LoginScreen appKey="admin-panel" .../>` componente compartido en `components/LoginScreen.tsx`
- **Roles aceptados**: Admin, Manager
